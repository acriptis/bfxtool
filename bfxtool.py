...
"""
Tool to display live BitFinex market info and
...
INI_DEFAULTS =  [["bfxtool", "set_xterm_title", "True"]
                ,["bfxtool", "dont_truncate_logfile", "False"]
                ,["bfxtool", "show_orderbook_stats", "True"]
                ,["bfxtool", "highlight_changes", "True"]
                ,["bfxtool", "orderbook_group", "0"]
                ,["bfxtool", "orderbook_sum_total", "False"]
                ,["bfxtool", "display_right", "history_chart"]
                ,["bfxtool", "depth_chart_group", "1"]
                ,["bfxtool", "depth_chart_sum_total", "True"]
                ,["bfxtool", "show_ticker", "True"]
                ,["bfxtool", "show_depth", "True"]
                ,["bfxtool", "show_trade", "True"]
                ,["bfxtool", "show_trade_own", "True"]
                ]
...
def try_get_lock_or_break_open():
    ...
    while time.time() < time_end:
        if bfxapi.Signal._lock.acquire(False):
            return
        time.sleep(0.001)
...
class WinConsole(Win):
    ...
    def __init__(self, stdscr, bfx):
        """create the console window and connect it to the Bfx debug
        callback function"""
        self.bfx = bfx
        bfx.signal_debug.connect(self.slot_debug)
    ...
    def slot_debug(self, dummy_bfx, (txt)):
    ...
    def write(self, txt):
        ...
        if " tick:" in txt:
            if not self.bfx.config.get_bool("bfxtool", "show_ticker"):
                return
        if "depth:" in txt:
            if not self.bfx.config.get_bool("bfxtool", "show_depth"):
                return
        if "trade:" in txt:
            if "own order" in txt:
                if not self.bfx.config.get_bool("bfxtool", "show_trade_own"):
                    return
            else:
                if not self.bfx.config.get_bool("bfxtool", "show_trade"):
                    return
...
class WinOrderBook(Win):
    ...
    def __init__(self, stdscr, bfx):
        ...
        onChanged callback of the bfx.orderbook instance"""
        self.bfx = bfx
        bfx.orderbook.signal_changed.connect(self.slot_changed)
        Win.__init__(self, stdscr)
    ...
    def paint(self):
        ...
        def paint_row(pos, price, vol, ownvol, color, changevol):
            ...
            if changevol > 0:
                ...
                col2 = col_vol
            self.addstr(pos, 0,  book.bfx.quote2str(price), color)
            self.addstr(pos, 12, book.bfx.base2str(vol), col2)
            if ownvol:
                self.addstr(pos, 28, book.bfx.base2str(ownvol), col_own)
        ...
        sum_total = bfx.config.get_bool("bfxtool", "orderbook_sum_total")
        group = bfx.config.get_float("bfxtool", "orderbook_group")
        group = bfx.quote2int(group)
        if group == 0:
            group = 1
        ...
        if len(book.asks):
            ...
            if bfx.config.get_bool("bfxtool", "highlight_changes"):
        ...
        if self.bfx.config.get_bool("bfxtool", "set_xterm_title"):
            last_candle = self.bfx.history.last_candle()
            if last_candle:
                title = self.bfx.quote2str(last_candle.cls).strip()
                title += " - bfxtool -"
                title += " bid:" + self.bfx.quote2str(book.bid).strip()
                title += " ask:" + self.bfx.quote2str(book.ask).strip()
...
class WinChart(Win):
    ...
    def __init__(self, stdscr, bfx):
        ...
        bfx.orderbook.signal_changed.connect(self.slot_orderbook_changed)
    ...
    def paint_y_label(self, posy, posx, price):
        ...
        fprice = self.bfx.quote2float(price)
        ...
        need_digits = int(math.log10(self.bfx.quote2float(self.pmax))) + 1
    ...
    def paint(self):
        typ = self.bfx.config.get_string("bfxtool", "display_right")
    ...
    def paint_depth_chart(self):
        ...
        if self.bfx.curr_quote in "JPY SEK":
            BAR_LEFT_EDGE = 7
            FORMAT_STRING = "%6.0f"
        else:
            BAR_LEFT_EDGE = 8
        ...
        def paint_depth(pos, price, vol, own, col_price, change):
            ...
            if change > 0:
                ...
                col = col_bar
            pricestr = FORMAT_STRING % self.bfx.quote2float(price)
        ...
        group = self.bfx.config.get_float("bfxtool", "depth_chart_group")
        if group == 0:
            group = 1
        group = self.bfx.quote2int(group)
        ...
        bin_bids = []
        mid = self.height / 2
        sum_total = self.bfx.config.get_bool("bfxtool", "depth_chart_sum_total")
        ...
        while pos < self.height and bin_price >= 0:
            ...
            bin_vol = self.bfx.base2int(bin_vol_quote / book.bid)
            if bin_vol > prev_vol:
                # add only non-empty bins
                if sum_total:
        ...
        if self.bfx.config.get_bool("bfxtool", "highlight_changes"):
...
class WinStatus(Win):
    ...
    def __init__(self, stdscr, bfx):
        ...
        bfx.signal_orderlag.connect(self.slot_orderlag)
        bfx.signal_wallet.connect(self.slot_changed)
        bfx.orderbook.signal_changed.connect(self.slot_changed)
        Win.__init__(self, stdscr)
    ...
    def sort_currency_list_if_changed(self):
        ...
        currency_list = self.bfx.wallet.keys()
        ...
        if self.bfx.curr_base in currency_list:
            currency_list.remove(self.bfx.curr_base)
        if self.bfx.curr_quote in currency_list:
            currency_list.remove(self.bfx.curr_quote)
        currency_list.sort(key=lambda name: -self.bfx.wallet[name])
        currency_list.insert(0, self.bfx.curr_quote)
        currency_list.insert(0, self.bfx.curr_base)
        self.sorted_currency_list = currency_list

    def paint(self):
        """paint the complete status"""
        cbase = self.bfx.curr_base
        cquote = self.bfx.curr_quote
        ...
        if len(self.sorted_currency_list):
            for currency in self.sorted_currency_list:
                if currency in self.bfx.wallet:
                    #line1 += currency + " " \
                    #+ bfxapi.int2str(self.bfx.wallet[currency], currency).strip() \
                    #+ " + "
                    line1 += currency + " " \
                    + "%f" % self.bfx.wallet[currency] \
                    + " + "
            line1 = line1.strip(" +")
        else:
            line1 += "No info (yet)"
        ...
        #
        line2 = ""
        if self.bfx.config.get_bool("bfxtool", "show_orderbook_stats"):
            str_btc = locale.format('%d', self.bfx.orderbook.total_ask, 1)
            str_fiat = locale.format('%d', self.bfx.orderbook.total_bid, 1)
            if self.bfx.orderbook.total_ask:
                str_ratio = locale.format('%1.2f',
                    self.bfx.orderbook.total_bid / self.bfx.orderbook.total_ask, 1)
            else:
                str_ratio = "-"
        ...
        #line2 += "s_lag: %.3f s" % (self.bfx.socket_lag / 1e6)
    ...
    def slot_changed(self, dummy_sender, dummy_data):
        """the callback funtion called by the Bfx() instance"""
        self.do_paint()
...
class DlgCancelOrders(DlgListItems):
    ...
    def init_items(self):
        for order in self.bfx.orderbook.owns:
    ...
    def paint_item(self, posy, index):
        ...
        self.addstr(posy, 9, self.bfx.quote2str(order.price), attr)
        self.addstr(posy, 22, self.bfx.base2str(order.volume), attr)
...
class TextBox():
    ...
    def modal(self):
        ...
        self.win.move(0, 0)
        self.editing = True
        bfxapi.start_thread(self.cursor_placement_thread, "TextBox cursor placement")
...
class DlgNewOrder(Win):
    ...
    def __init__(self, stdscr, bfx, color, title):
        self.bfx = bfx
        self.color = color
        self.title = title
    ...
    def paint(self):
        ...
        self.addstr(2, 30, self.bfx.curr_quote)
        self.addstr(4, 2, "volume", self.color)
        self.addstr(4, 30, self.bfx.curr_base)
...
class DlgNewOrderBid(DlgNewOrder):
    ...
    def __init__(self, stdscr, bfx):
        DlgNewOrder.__init__(self, stdscr, bfx,
            COLOR_PAIR["dialog_bid_text"],
            "New buy order")

    def do_submit(self, price, volume):
        self.bfx.buy(price, volume)
...
class LogWriter():
    """connects to bfx.signal_debug and logs it all to the logfile"""
    def __init__(self, bfx):
        self.bfx = bfx
        if self.bfx.config.get_bool("bfxtool", "dont_truncate_logfile"):
            logfilemode = 'a'
        else:
            logfilemode = 'w'
        ...
        self.bfx.signal_debug.connect(self.slot_debug)
...
class PrintHook():
    """intercept stdout/stderr and send it all to bfx.signal_debug instead"""
    def __init__(self, bfx):
        self.bfx = bfx
    ...
    def write(self, string):
        """called when someone uses print(), send it to bfx"""
        string = string.strip()
        if string != "":
            self.bfx.signal_debug(self, string)
...
class StrategyManager():
    ...
    def __init__(self, bfx, strategy_name_list):
    ...
    def unload(self):
        ...
        self.bfx.signal_strategy_unload(self, None)
        self.strategy_object_list = []
...
def toggle_setting(bfx, alternatives, option_name, direction):
    ...
    # pylint: disable=W0212
    with bfxapi.Signal._lock:
        setting = bfx.config.get_string("bfxtool", option_name)
        try:
            newindex = (alternatives.index(setting) + direction) % len(alternatives)
        except ValueError:
            newindex = 0
        bfx.config.set("bfxtool", option_name, alternatives[newindex])
        bfx.config.save()

def toggle_depth_group(bfx, direction):
    """toggle the step width of the depth chart"""
    if bfx.curr_quote in "JPY SEK":
        alt = ["5", "10", "25", "50", "100", "200", "500", "1000", "2000", "5000", "10000"]
    else:
        alt = ["0.05", "0.1", "0.25", "0.5", "1", "2", "5", "10", "20", "50", "100"]
    toggle_setting(bfx, alt, "depth_chart_group", direction)
    bfx.orderbook.signal_changed(bfx.orderbook, None)

def toggle_orderbook_group(bfx, direction):
    ...
    toggle_setting(bfx, alt, "orderbook_group", direction)
    bfx.orderbook.signal_changed(bfx.orderbook, None)

def toggle_orderbook_sum(bfx):
    """toggle the summing in the orderbook on and off"""
    alt = ["False", "True"]
    toggle_setting(bfx, alt, "orderbook_sum_total", 1)
    bfx.orderbook.signal_changed(bfx.orderbook, None)

def toggle_depth_sum(bfx):
    """toggle the summing in the depth chart on and off"""
    alt = ["False", "True"]
    toggle_setting(bfx, alt, "depth_chart_sum_total", 1)
    bfx.orderbook.signal_changed(bfx.orderbook, None)

def set_ini(bfx, setting, value, signal, signal_sender, signal_params):
    """set the ini value and then send a signal"""
    # pylint: disable=W0212
    with bfxapi.Signal._lock:
        bfx.config.set("bfxtool", setting, value)
        bfx.config.save()
...
def main():
    ...
    def curses_loop(stdscr):
        ...
        try:
            init_colors()
            bfx = bfxapi.Bfx(secret, config)

            logwriter = LogWriter(bfx)
            printhook = PrintHook(bfx)

            conwin = WinConsole(stdscr, bfx)
            bookwin = WinOrderBook(stdscr, bfx)
            statuswin = WinStatus(stdscr, bfx)
            chartwin = WinChart(stdscr, bfx)

            strategy_manager = StrategyManager(bfx, strat_mod_list)
            ...
            while True:
                ...
                if key == ord("q"):
                    ...
                    DlgNewOrderBid(stdscr, bfx).modal()
                elif key == curses.KEY_F5:
                    DlgNewOrderAsk(stdscr, bfx).modal()
                elif key == curses.KEY_F6:
                    DlgCancelOrders(stdscr, bfx).modal()
                ...
                # which chart to show on the right side
                    ...
                    set_ini(bfx, "display_right", "history_chart",
                        bfx.history.signal_changed, bfx.history, None)
                elif key == ord("D"):
                    set_ini(bfx, "display_right", "depth_chart",
                        bfx.orderbook.signal_changed, bfx.orderbook, None)

                #  depth chart step
                elif key == ord(","): # zoom out
                    toggle_depth_group(bfx, +1)
                elif key == ord("."): # zoom in
                    toggle_depth_group(bfx, -1)

                # orderbook grouping step
                elif key == ord("-"): # zoom out (larger step)
                    toggle_orderbook_group(bfx, +1)
                elif key == ord("+"): # zoom in (smaller step)
                    toggle_orderbook_group(bfx, -1)

                elif key == ord("S"):
                    toggle_orderbook_sum(bfx)

                elif key == ord("T"):
                    toggle_depth_sum(bfx)

                # lowercase keys go to the strategy module
                elif key >= ord("a") and key <= ord("z"):
                    bfx.signal_keypress(bfx, (key))
                else:
                    bfx.debug("key pressed: key=%i" % key)
    ...
    argp = argparse.ArgumentParser(description='BitFinex live market data monitor'
    ...
    config = bfxapi.BfxConfig("bfxtool.ini")
    config.init_defaults(INI_DEFAULTS)
    secret = bfxapi.Secret(config)
    ...
    if args.add_secret:
        ...
        bfxapi.FORCE_PROTOCOL = args.protocol
        bfxapi.FORCE_NO_FULLDEPTH = args.no_fulldepth
        bfxapi.FORCE_NO_DEPTH = args.no_depth
        bfxapi.FORCE_NO_LAG = args.no_lag
        bfxapi.FORCE_NO_HISTORY = args.no_history
        bfxapi.FORCE_HTTP_API = args.use_http
        bfxapi.FORCE_NO_HTTP_API = args.no_http
        if bfxapi.FORCE_NO_DEPTH:
            bfxapi.FORCE_NO_FULLDEPTH = True
        ...
        if secret.prompt_decrypt() != secret.S_FAIL_FATAL:
            ...
            if len(debug_tb):
                ...
                print "*******************************************************"
                print "*  Please donate                                      *"
                print "*  @prof7bit:      1C8aDabADaYvTKvCAG1htqYcEgpAhkeYoW *"
                print "*  @acriptis:      18oSVDtLgHenUAepDXwy1XL3VD8qg3neBG *"
                print "*******************************************************"
                print "*  See usage hints: http://prof7bit.github.io/goxtool/*"
                print "*******************************************************"
